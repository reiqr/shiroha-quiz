...
## v9 新增 v89_19_50～v89_19_52 边界回归

- 解析正文中的编号分步继续保护，不能把“1. 先... / 2. 再...”误切成新题。
- 上一题解析后紧跟标准填空题或判断题时，应按标准题结构恢复解析。
- 选择题答案兼容 `A-D`、`A至D`、`全选`、`以上全选`，并按当前题实际选项展开。
- 集中答案区中的范围答案和全选答案应合并回对应多选题。
- `A型血`、`维生素B12` 等正文缩写不能误触发括号答案或答案解析规则。
- 题干括号多选答案兼容 `A、B`、`A，B`、`A,B`、`A B`。
- 集中答案区后遇到填空题章节或判断题章节时，应退出答案区跳过状态并恢复后续题。
- 真正的 `答案解析` 区仍应只作为答案/解析来源，不能被恢复成新题。
