...
如果原题库格式非常混乱，可以先做一次格式清洗再导入。国内用户可优先使用 DeepSeek、GLM / 智谱清言等工具辅助整理；如果可以稳定使用海外模型，也推荐 ChatGPT、Claude。**请注意：隐私风险**主要来自"把题库上传到外部 AI 工具清洗"，不是 Shiroha Quiz 本地导入本身。清洗目标只是统一题号、选项、答案和解析格式，不能改题意、不能编造答案，无法确认的答案应标为"待确认"。
