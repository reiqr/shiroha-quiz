...
<table>
  ...
  <tr><td>使用形态</td><td>安装 APK（Android 8.0+，即 API 26+）</td><td>解压后双击启动器，免安装</td></tr>
  ...
  <tr><td>平台特色</td><td>手机端：暗夜模式、平板侧边导航、系统返回键<br>Shiroha 模式、斩题、背题、智能复习</td><td>电脑端：单题 AI 解析按题型分层<br>OCR 测试区可将扫描件转文本并导出 DOCX</td></tr>
...
> **怎么读这一节**：只有[刷题与考试](#刷题与考试)下的四个子小节按「双端通用 / 原生版专属」分段；其余小节是平铺列表，平台差异在条目内用「原生版…」「Web 版…」标注。两端的整体差异见[原生版与 Web 版区别](#原生版与-web-版区别)——那只是**粗略对比**，更细的差异以本节为准。
>
> **内置题库**：Web 版预置 `C1驾照科目一模拟练习题库`（393 题）作为默认题库，便于首次体验；题目为公开科目一常见考点整理，**不作为官方最新全量题库**。
