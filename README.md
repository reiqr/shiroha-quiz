...
- **Web 版**：适合电脑整理题库，支持导入、练习、考试、错题、备份、AI 与 OCR。
- **Android 原生版**：当前主推，Kotlin + Compose 原生实现，支持 AI、多空填空、背题/斩题、图片题和智能复习。

<img src="assets/shiroha_quiz_ui_assets_v2_cutout/illus_me_settings.webp" width="150" align="right" alt="看板娘" />
...
原生版更偏向**手机端长期刷题和复习**，也是目前主要开发线。

- **练习体验**：支持普通、随机、顺序、即时反馈和批量练习；可以背题、斩题、打乱选项、调整字号，并保留独立的顺序练习进度。
- **错题与复习**：错题本支持当前题库 / 全部题库作用域、掌握状态和智能复习；学习记录可逐题复盘并筛选错题。
- **导入与图片题**：支持 DOCX 内嵌图片提取、材料题 / 共用题干、集中答案区等复杂格式，也支持图片题和带图片的 ZIP 备份。
- **文档识别**：可使用 MinerU 在线 OCR 处理扫描 PDF，并支持图片提取与人工绑定。
- **AI**：支持导入核对、补解析、批量范围处理、单题解析与追问；还可选启用无答案题 AI 临时参考答案。
- **备份**：支持 WebDAV 手动 / 自动备份与恢复，API Key 使用 Android Keystore 加密保存。
- **界面**：Compose + Material3 原生界面，支持浅色 / 暗夜、Shiroha 模式和平板侧边导航。

### Web 版

Web 版更偏向**电脑端题库导入、清洗和快速刷题**，适合处理原始题库文件。

- **练习与考试**：支持普通、随机、顺序练习，以及考试、错题、收藏和学习记录等基础学习流程。
- **题库整理**：支持全文查找 / 替换、导入预览和手动修正，适合先在电脑上整理较乱的题库。
- **文档处理**：支持 PDF.js 解析文字层 PDF，扫描 PDF 可使用 OCR 测试区转成文本或 DOCX。
- **AI**：支持 AI 整理、核对、补解析和结构化导入；单题 AI 会按题型生成解析，并支持连续追问后保存完整解析草稿。
- **本地模型**：除 DeepSeek、OpenAI 兼容和自定义接口外，还可连接 Ollama / LM Studio 等本机服务。
- **备份**：支持 WebDAV 上传、远程列表、校验以及合并 / 覆盖恢复。
- **使用方式**：无需安装 App，下载 Web ZIP 后通过本地启动器运行，也可以使用在线版。

### 两端互通

Web 与原生版的题库和备份格式持续保持互通；普通 JSON 可以跨端导入，含图片题时建议使用 ZIP 保留素材。具体功能变化以 [CHANGELOG](./CHANGELOG.md) 和 [Releases](https://github.com/reiqr/shiroha-quiz/releases) 为准。
...
- **题型**：单选、多选、判断、填空（含多空）、简答题。
- **练习**：随机练习、顺序练习、考试、错题复习、收藏与学习记录。
- **范围与进度**：支持题库分组、跨题库练习范围和顺序练习进度。
- **原生特色**：即时/批量练习、背题、斩题、选项打乱、字号调整和智能复习。
- **考试**：支持倒计时、答题卡、交卷提醒和结果统计。
- **快速编辑**：练习中可直接修改题目并写回来源题库。

### 题库导入

- **格式**：支持 `docx`、`xlsx` / `csv`、`txt`、`json`、文字层 `pdf` 和粘贴文本。
- **双文件导入**：支持“题目文件 + 答案文件”自动匹配。
- **自动解析**：识别题号、题干、选项、答案、解析、题型和分区。
- **核对与修正**：支持导入预览、异常提示、手动修正、全文查找/替换。
- **原生版**：支持 DOCX 内嵌图片提取、材料题/共用题干和集中答案区等复杂格式。
- **Web 版**：支持 PDF.js 文字层解析，并提供扫描 PDF OCR 测试兜底。
- 原生解析器持续维护真实题库回归测试，复杂边界说明放在专项文档中。

### AI 辅助

- **AI 导入**：Web 与原生版均支持整理 / 核对 / 补解析，优先使用结构化题目结果。
- **单题 AI**：练习页可按需解析与追问，确认后保存解析。
- **临时参考答案**：原生版可选启用，默认关闭，不写回题库、不进入错题本。
- **接口**：支持 DeepSeek、OpenAI 兼容接口和自定义接口；Web 版也可连接 Ollama / LM Studio。
...
- **本地备份**：支持完整数据备份和恢复。
- **跨端互通**：Web 与原生版题库格式互通，图片题可通过 ZIP 保留素材。
- **WebDAV**：支持云备份与远程恢复，原生版还可选自动备份。
- **密钥存储**：原生版 API Key 使用 Android Keystore 加密保存。
...
2. 在 **导入** 页面选择文件或粘贴文本。
3. 在 **核对页** 检查识别结果，必要时手动修正或使用 AI 辅助。
4. 选择 **练习、考试、背题** 等模式开始使用。
5. 错题、收藏、学习记录和智能复习会保存在本机。

### 数据备份建议

- **重要题库导入后**及时导出备份。
- **换设备、清缓存、卸载 App 前**先备份。
