...
- **原生版 AI 补解析（编辑器）**：题库编辑、审阅、导入预览、快速编辑等入口统一集成，一键生成解析建议。导入页 AI 重构默认优先生成结构化题目——**直接产出题干、选项、答案、解析等字段，不再让 AI 清洗后的文本重新走一遍本地切块解析**，因此不会因为原文里的特殊文本（编号、代码片段、多行答案等）触发切块误判而导致题目解析出错；只有结构化校验不通过时才退回标准文本重解析，并在预览中提示题量变化供人工核对。
